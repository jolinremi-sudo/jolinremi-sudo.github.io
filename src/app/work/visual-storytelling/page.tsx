  ...
  return <main className={`case-study ${styles.page}`}>
    ...
    <section className={styles.opening}>
      ...
      <div className={styles.openingGrid}>
        ...
        <figure className={styles.heroImage}><span className={styles.bluePaper} aria-hidden="true" /><Image src={asset("visual-09-summer.webp")} alt="Person carrying a personalized bag by the beach" fill priority sizes="(max-width: 760px) 88vw, 48vw" /><figcaption>SUMMER / IN USE</figcaption></figure>
      </div>
    </section>
    ...
    <section className={styles.idea}>
      ...
      <div className={`${styles.ideaImages} ${refine.ideaImages}`}><figure className={`${styles.emotion} ${refine.emotion}`}><Image src={asset("visual-08-emotion.webp")} alt="Child interacting with a personalized pillow" fill sizes="(max-width: 760px) 88vw, 42vw" /></figure><figure className={`${styles.bedroom} ${refine.bedroom}`}><Image src={asset("visual-07-bedroom.webp")} alt="Personalized pillow in a bedroom setting" fill sizes="(max-width: 760px) 68vw, 26vw" /><figcaption>PERSON / CONTEXT</figcaption></figure></div>
    </section>
    ...
    <section className={`${styles.context} ${refine.contextSection}`}>
      ...
      <div className={`${styles.contextWall} ${refine.contextWall}`}>{[["visual-01-outdoor.webp","EVERYDAY","outdoor"],["visual-02-lifestyle.webp","MOVEMENT","lifestyle"],["visual-03-holiday.webp","HOLIDAY","holiday"],["visual-04-garden.webp","OUTDOOR","garden"]].map(([src,label,className])=><figure className={styles[className]} key={src}><Image src={asset(src)} alt={`${label.toLowerCase()} lifestyle product context`} fill sizes="(max-width: 760px) 88vw, 28vw" /><figcaption>{label}</figcaption></figure>)}</div>
    </section>

    <section className={styles.styling}>
      <div className={styles.stylingImages}><figure><Image src={asset("visual-05-kids.webp")} alt="Children using personalized products" fill sizes="(max-width: 760px) 88vw, 38vw" /></figure><figure><Image src={asset("visual-06-styling.webp")} alt="Styled personalized product scene" fill sizes="(max-width: 760px) 74vw, 28vw" /></figure></div>
    ...
    <section className={styles.system}>
      ...
      <div className={styles.systemWall}>{[["visual-10-soft-lifestyle.webp","soft"],["visual-02-lifestyle.webp","life"],["visual-06-styling.webp","style"],["visual-08-emotion.webp","warm"],["visual-03-holiday.webp","holidayAgain"]].map(([src,className])=><figure className={styles[className]} key={src}><Image src={asset(src)} alt="Lifestyle visual storytelling example" fill sizes="(max-width: 760px) 88vw, 32vw" /></figure>)}<span>SOFT LIGHT / REAL LIFE / PERSONAL DETAILS</span></div>
    </section>

    <section className={styles.reflection}><div className={styles.reflectionInner}><div><p className={styles.label}>06 / REFLECTION</p><h2>GOOD VISUAL CONTENT<br />DOESN&apos;T JUST MAKE<br />A PRODUCT LOOK BETTER.</h2></div><div><p className={styles.takeaway}>It helps people see<br />a place for it<br />in their lives.</p><p className={styles.chinese}>这段实践让我逐渐形成了一套自己的视觉判断：<br /><br />场景不是商品的背景，<br />而是帮助消费者理解商品、<br />想象使用方式并建立情感连接的一部分。</p></div><p className={styles.sequence}>PRODUCT <span>→</span> CONTEXT <span>→</span> IMAGINATION <span>→</span> CONNECTION</p></div><Link className={styles.nextCase} href="/#work" aria-label="Back to Selected Work"><span>BACK TO WORK ↖</span><strong>EXPLORE<br />SELECTED WORK</strong><small>01 / 02 / 03 — CASE ARCHIVE</small></Link></section>
  </main>;
}
